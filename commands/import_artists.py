    ...
    for aka in data:
        ...
        if found_artist is not None:
            found_aka = ArtistAka.query.filter_by(
                artist_mbid=aka["mbid"], name=aka["aka_name"]
            ).first()
            if found_aka is None:
                print(
                    "ADDED AKA: {} - AKA: {}".format(found_artist.name, aka["aka_name"])
                )
                new_aka = ArtistAka(artist_mbid=found_artist.mbid, name=aka["aka_name"])
