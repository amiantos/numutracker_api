    ...
    if object.art is False and type(object) is Release:
        for artist in object.artists:
            if artist.art is True:
                return {
                    'thumb': 'https://numu.sfo2.digitaloceanspaces.com/artist/thumb/{}.png'.format(artist.mbid),
                    'full': 'https://numu.sfo2.digitaloceanspaces.com/artist/full/{}.png'.format(artist.mbid),
                    'large': 'https://numu.sfo2.digitaloceanspaces.com/artist/large/{}.png'.format(artist.mbid)
                }
...
def user_artist(user_artist):
    total_releases = Release.query.filter(
        Release.mbid.in_(
            db.session.query(ArtistRelease.release_mbid).filter(
                ArtistRelease.artist_mbid == user_artist.mbid
            ).all()
        ),
